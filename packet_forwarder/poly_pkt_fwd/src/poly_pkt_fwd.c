...
int main(void)
{
	...
	if (radiostream_enabled == true) {
		...
		if (i == LGW_HAL_SUCCESS) {
			...
			MSG("ERROR: [main] failed to start the concentrator (radiostream) \n");
			exit(EXIT_FAILURE);
		}
	} else {
	...
	*(uint32_t *)(buff_up + 8) = net_mac_l;
	...
	while (!exit_sig && !quit_sig) {
		...
		for (i=0; i < nb_pkt; ++i) {
			...
			if(p->status == 16) {
				MSG(" %s || Concentrator %s || size %u, modulation %u, BW %u, DR %u, RSSI %.1f\n", fetch_timestamp, SPI_DEV_PATH, p->status, p->size, p->modulation, p->bandwidth, p->datarate, p->rssi);
			}
